...
public class BrowserUtils {
    ...
    public static void switchWindowAndVerify(String expectedInUrl, String expectedInTitle) {
        ...
        for (String each : allWindowsHandles) {
            ...
            Driver.getDriver().switchTo().window(each);//switches to each window via windowhandle in the set

            System.out.println("Current URL: " + Driver.getDriver().getCurrentUrl());

            if (Driver.getDriver().getCurrentUrl().contains(expectedInUrl)) {//Stops at the current switched window if the condition occurs
                break;
            }
        }

        //5. Assert:Title contains “expectedInTitle”
        String actualTitle = Driver.getDriver().getTitle();//Takes the title of the current switched window
    ...
    public static void waitForInvisibilityOf(WebElement webElement) {
        ...
        wait.until(ExpectedConditions.invisibilityOf(webElement));//Argument type is webElement
    }
